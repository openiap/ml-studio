<template>
  ...
  <v-text-field v-model="node.data.inputDim" :rules="posetiv" label="Input dimension" type="number">
    <template v-slot:append>
      <v-tooltip text="Integer > 0. Dimensionality of the input (integer). This option (or alternatively, the option inputShape) is required when this layer is used as the first layer in a model.">
        <template v-slot:activator="{ props }">
          <v-icon v-bind="props" color="primary">mdi-information</v-icon>
        </template>
      </v-tooltip>
