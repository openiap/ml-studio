<template>
  API operation {{ node.id }}
  <v-text-field v-model="node.label" :counter="30" :rules="nameRules" label="Name" required />
  <v-text-field v-model="node.id" label="id" readonly />
  <v-text-field v-model="node.data.op" label="Operation" readonly />
  <v-text-field v-model="node.data.category" label="Category" readonly />
</template>
