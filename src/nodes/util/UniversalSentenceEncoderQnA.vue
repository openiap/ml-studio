...
if(node.data == null || Object.keys(node.data).length == 0) {
  node.data = {};
  node.data.queries = "queries";
  node.data.responses = "responses";
  node.data.contexts = "contexts";
  node.data.tokenizer = "tokenizer";
}
...
<style>
.toolbox-node-json {
  background: var(--ml-studio-json-node-background-color);
  color: var(--ml-studio-json-node-font-color);
}
</style>
<style scoped>
.node-json {
