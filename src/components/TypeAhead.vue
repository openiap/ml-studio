...
export default /*#__PURE__*/ defineComponent({
    ...
    mounted() {
        if (this.defaultItem != null) {
            this.selectItem(this.defaultItem, false);
            // console.debug('TypeAhead: mounted with value ' + this.defaultItem?._id + ' from ' + this.defaultItem?._id);
        } else if (this.modelValue != null) {
            this.selectItem(this.modelValue, false);
            // console.debug('TypeAhead: mounted with value ' + this.modelValue?._id + ' from ' + this.modelValue?._id);
        } else {
    ...
    watch: {
        modelValue(newValue, oldValue) {
            // console.debug('TypeAhead: defaultItem changed to ' + newValue?._id + ' from ' + oldValue?._id);
            this.input = this.itemProjection(newValue);
        },
        modelValue(newValue, oldValue) {
            // console.debug('TypeAhead: modelValue changed to ' + newValue?._id + ' from ' + oldValue?._id);
            this.input = this.itemProjection(this.modelValue);
        },
        items() {
    ...
    methods: {
        ...
        scrollSelectionIntoView() {
            setTimeout(() => {
                ...
                if (active_node == null || list_node == null) return;;
        ...
        selectItem(item, emit = true) {
            ...
            this.$refs.inputRef.blur();
            if (emit) this.$emit('selectItem', item);
