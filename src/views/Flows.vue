<template>
  <div class="card">
    <div class="row">
      <div class="col">
        <v-text-field ref="searchfield" label="Search for workflow by name or using json query" @keyup="DoSearch" v-model="searchValue" />
      </div>
      <div class="col">
  ...
  <div class="card">
    <EasyDataTable :headers="headers" :items="items" :loading="loading" :server-items-length="serverItemsLength"
      ...
      <template #item-name="item">
        <router-link :to="{ name: 'FlowWithId', params: { id: item._id }}">{{item.name}}</router-link>
      </template>
      <template #item-operation="item">
        <div class="operation-wrapper">
          <router-link :to="{ name: 'FlowWithId', params: { id: item._id }}">Open</router-link>
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      this.$router.push({ name: 'FlowWithId', params: { id: item._id } });
    },
    openlinkItem(item) {
    ...
    Insert() {
      this.$router.push({ name: 'Flow', params: {  } });
    },
    RowClick(item) {
